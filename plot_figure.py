...
def plot_one_group_bar_figure(data, labels_name=None, ax=None, width=0.5, colors=None, title_name='', title_fontsize=10, title_pad=20, x_label_name='', x_label_fontsize=10, x_tick_fontsize=10, x_tick_rotation=0, x_label_ha='center', y_label_name='', y_label_fontsize=10, y_tick_fontsize=10, y_tick_rotation=0, y_max_tick_to_one=False, y_max_tick_to_value=1, math_text=True, one_decimal_place=False, percentage=False, ax_min_is_0=False, asterisk_fontsize=10, multicorrect=False, statistic=False, test_method='ttese_ind', tails='two-sided', **kwargs):
    # # 设置部分默认值
    if ax is None:
        ax = plt.gca()
        ...
        for i1 in range(len(labels_name)):
            for i2 in range(i1+1, len(labels_name)):
                if test_method == 'ttest_ind':
                    ...
                    globals()['s_' + str(i1) + '_' + str(i2)], globals()['t_' + str(i1) + '_' + str(i2)] = stats.mannwhitneyu(data[i1], data[i2], alternative=tails)
                elif test_method == 'permutation_mean':
                    def per_statistic(x, y):
                        return np.mean(x) - np.mean(y)
                    res = stats.permutation_test((data[i1], data[i2]), per_statistic, permutation_type='independent', n_resamples=10000, alternative=tails)
...
def plot_matrix_figure(data, row_labels_name=[], col_labels_name=[], ax=None, cmap='bwr', cbarlabel_name='', cbarlabel_pad=0.1, cbartick_fontsize=10, cbartick_rotation=0, x_rotation=60, row_labels_fontsize=5, col_labels_fontsize=5, colorbar=True,  cbarlabel_fontsize=10, title_name='', title_fontsize=15, title_pad=20, vmax=None, vmin=None, aspect='equal', **kwargs):
    """
    ## 说明:
    该函数用于绘制一个矩阵图。
    ...
    if colorbar:
        ...
        cax = divider.append_axes("right", size="5%", pad=cbarlabel_pad)  # pad 控制 colorbar 与图的距离
        # 创建colorbar
...
def plot_one_group_violin_figure(data, test_method='ttest_ind', labels_name=None, ax=None, width=0.5, colors=None, title_name='', title_fontsize=10, title_pad=20, x_label_name='', x_label_fontsize=10, x_tick_fontsize=10, x_tick_rotation=0, x_label_ha='center', y_label_name='', y_label_fontsize=10, y_tick_fontsize=10, y_tick_rotation=0, y_max_tick_to_one=False, y_max_tick_to_value=1, math_text=True, one_decimal_place=False, percentage=False, ax_min_is_0=False, asterisk_fontsize=10, multicorrect=False, y_lim_range=None, statistic=False, tails='two-sided', **kwargs):
    # 设置部分默认值
    if ax is None:
        ax = plt.gca()
    if labels_name is None:
...
def plot_macaque_hemi_brain_figure(data, ax_direction='horizontal', hemi='lh', cmap='Reds', surf='inflated', vmin=None, vmax=None, colorbar=True, colorbar_direction='vertical', colorbar_label_name="AAA", colorbar_label_fontsize=10, colorbar_tick_fontsize=10, colorbar_tick_rotation=0, colorbar_outline=False):
    # 定义路径
    current_dir = os.path.dirname(os.path.abspath(__file__))
    subj_dir = op.join(current_dir, 'FS')
    subj = "NMT"
